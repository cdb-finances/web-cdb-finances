...
function ModalRegisterClient() {
  const { setPopupMessage, setIsPopup,
    currentClient, setIsModalEditRegisterClient,
  ...
  async function updateClient() {
    ...
    try {
      ...
      setIsModalEditRegisterClient(false);
        ...
        <form onSubmit={(event) => preventDefault(event)}>
          <div className="flex-row justify-between">
            <div className="flex-row modal-title">
              ...
              <span className="width100 gray1 title1">Editar cliente</span>
            </div>
            <img
              style={{ cursor: 'pointer' }}
              src={closeIcon}
              alt="close icon"
              onClick={() => setIsModalEditRegisterClient(false)}
            />
          </div>
